...
class MPPII(SamplingBasedController):
    """Model-predictive path integral control.

    Implements "MPPII-generic" as described in https://arxiv.org/abs/2409.07563.
    Unlike the original MPPII derivation, this does not assume stochastic,
    ...
    def init_params(self, seed: int = 0) -> MPPIIParams:
        """Initialize the policy parameters."""
        rng = jax.random.key(seed)
        mean = jnp.zeros((self.task.planning_horizon, 6))
        return MPPIIParams(mean=mean, rng=rng)

    def sample_controls(
        self, params: MPPIIParams
    ) -> Tuple[jax.Array, MPPIIParams]:
    ...
    def update_params(
        self, params: MPPIIParams, rollouts: Trajectory
    ) -> MPPIIParams:
    ...
    def get_action(self, params: MPPIIParams, t: float) -> jax.Array:
