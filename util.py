...
def mat_to_quat(R):
    ...
    # Condition masks
    cond1 = trace > 0
    cond2 = jnp.logical_and(jnp.logical_not(cond1), 
                           jnp.logical_and(R[0, 0] > R[1, 1], R[0, 0] > R[2, 2]))
    cond3 = jnp.logical_and(jnp.logical_not(cond1), 
                           jnp.logical_and(jnp.logical_not(cond2), R[1, 1] > R[2, 2]))
    
    # Select w based on conditions
    w = jnp.where(cond1, w1, 
         jnp.where(cond2, w2, 
         jnp.where(cond3, w3, w4)))
    
    # Select x based on conditions
    x = jnp.where(cond1, x1, 
         jnp.where(cond2, x2, 
         jnp.where(cond3, x3, x4)))
    
    # Select y based on conditions
    y = jnp.where(cond1, y1, 
         jnp.where(cond2, y2, 
         jnp.where(cond3, y3, y4)))
    
    # Select z based on conditions
    z = jnp.where(cond1, z1, 
         jnp.where(cond2, z2, 
         jnp.where(cond3, z3, z4)))
